...
let allowedConfigs = new Set(['dev', 'production', 'betaproduction', 'test']);
...
const customRequestLogger = require('./libs/request_logger');
...
app.get('/docs', function (req, res) {
    ...
    if (!allowedConfigs.has(process.env.NODE_ENV)) {
        return res.send('ScrumBot for reports');
    }
...
app.get('/heartbeat', function (req, res, next) {
    db.connection.query('SELECT 1 from DUAL WHERE 1 =1 ', function (err, result) {
        ...
        res.send('ScrumBot for reports working fine ;)');
    });
});
...
process.on("uncaughtException", function (err) {
    console.error(utils.getLoggingTime() + " uncaughtException: " + err.message);
